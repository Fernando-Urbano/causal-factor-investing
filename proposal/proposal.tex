...
\setboolean{bool:showquestions}{false}
...
\definecolor{annotation}{RGB}{20, 200, 20}
...
\begin{colorparagraph}{annotation}
\section*{Which questions can we answer?}
\begin{itemize}
    \item Which model performs better for different ratio between $d_c$ and $d_a$? Meaning the amount of covariates that are relevant and irrelevant.
    \item Which model performs better in general?
    \item Which model has the best convergence rate (different sample sizes)?
    \item Which model performs better for different noise levels?
    \item Which models performs better for different levels of sparsity (amount of non-causal covariates)?
    \item Is the order of performance the same in terms of bias and variance?
    \item Which model performs better when there is mispecification?
\end{itemize}
\end{colorparagraph}
...
Ideally, the project should provide actionable recommendations for selecting the most appropriate DML method based on the causal scenario, data structure, noise levels, and sample size, sparsity of important covariates. Additionally, we aim to create a benchmarking framework that practitioners can use to evaluate and compare causal inference estimates derived from the tested methods in different scenarios.
...
\item $X_i \in \mathbb{R}^d$ is a vector of covariates.
\end{itemize}
...
$f(C_i)$ is a generic non-linear function explained in Section~\ref{subsubsec:f_c_non_linear_relationships}.
...
The true treatment effect is generated from:
...
\begin{equation}
    \tau = \mathbb{E}[\tau(C_i)] = \frac{1}{n} \sum_{i=1}^{n} \tau(C_i)
\end{equation}
...
\begin{equation}
    Y_i = Y_{i, 0} + \tau(C_i) \cdot T_i + \varepsilon_{y, i}
\end{equation}

where $Y_{i, 0}$ is the potential outcome if the treatment $T_i$ was not applied, $\tau(C_i)$ is the treatment effect, and $\varepsilon_{y, i}$ is a random error term generated from a normal distribution with mean 0 and variance $\sigma_{y}$.
...
\begin{equation}
    Y_i(1) = Y_{i, 0} + \tau(C_i), \quad Y_i(0) = Y_{i, 0},
...
Where $C_{j}$ is the the vector of the $j$-th causal confounder, $\beta_j$ is a random coefficient drawn from a uniform distribution $U(-1, 1)$, and $q_j(C_{j})$ is a random transformation of the $j$-th causal confounder. More specifically, the random transformation $q_j(C_{j})$ is drawn from the following set of possible transformations:
...
For each simulation $f(C)$ is calculated with some of the specified transformation sets and $\beta$'s three times: one for the calculation of $Y_{i, 0}$, one for the calculation of $\mathbb{P}[T_i]$, and one for the calculation of $\tau(C_i)$.
